//! Quantized Qwen3-VL Model combining GGUF LLM with vision encoder.
//!
//! This model loads:
//! - LLM weights from main GGUF file (qwen3vl architecture, `blk.{i}.*` tensors)
//! - Vision encoder from mmproj GGUF file (clip architecture, `v.blk.{i}.*` tensors)
//!
//! Architecture: qwen3vl / qwen3vlmoe
...
// Qwen3-VL LLM config extracted from GGUF metadata
...
fn verify_qwen3vl_arch(
    ...
    // Accept both qwen3vl and qwen3vlmoe architectures
    if actual_arch != "qwen3vl" && actual_arch != "qwen3vlmoe" {
        candle_core::bail!("Expected `qwen3vl` or `qwen3vlmoe` architecture, got `{actual_arch}`.");
    }
    Ok(actual_arch)
}
...
impl TryFrom<ContentMetadata<'_>> for PropsGGUF {
    ...
    fn try_from(c: ContentMetadata) -> std::result::Result<Self, Self::Error> {
        let _ = verify_qwen3vl_arch(c.metadata)?;
...
/// Combined Qwen3-VL model with LLM text weights and vision encoder.
...
impl ModelConfig::FromGGUF for ModelWeights {
    fn from_gguf<R: std::io::Seek + std::io::Read>(
        ...
        let actual_arch = verify_qwen3vl_arch(meta)?;
...
impl ModelWeights {
    ...
    ///     pixel_values: Image patches [total_patches, patch_dim] (optional)
    ///     image_grid_thw: Grid dimensions [num_images, 3] with (t, h, w) per image (optional)
    ///     image_positions: List of (batch_idx, start, end) for each image's placeholder tokens
    ///     start_offsets: Cumulative sequence offsets for KV cache
    ///     context_lens: Context lengths for logit extraction
    ///     metadata: Paged attention metadata (optional)
